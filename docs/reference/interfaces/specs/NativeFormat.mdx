...
### Temporary block streams {#temporary-block-streams}

Internal temporary block streams, including external sorting and rebuilt-projection merge spills, are sequences of the `Native` blocks described here. The internal holder writes and reads them at `DBMS_TCP_PROTOCOL_VERSION`. They have no native-TCP packet envelope or table-name prefix.

The holder turns constant header columns into full columns because constants are not serializable in this stream; it retains lazy columns so the reader sees the same header. Callers finish writing before opening a reader; an unfinished temporary buffer is canceled on destruction and releases its accounted storage. The temporary-data buffer wraps the `Native` sequence in its configured compression codec, so a reader first removes that compression and then reads ordinary `Native` blocks. A rebuilt projection may use such a stream only for a bounded prefix while choosing its final `Wide` or `Compact` part format; the spill is not a `MergeTree` part format and does not change the `Native` block layout. This is an internal storage convention, not an additional client-facing `FORMAT Native` negotiation mode.
